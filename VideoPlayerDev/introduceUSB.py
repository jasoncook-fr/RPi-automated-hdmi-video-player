...
def centerFileText(myText):
    return myText.get_rect(center=(display_width/2, (display_height/2)-font_size))
    ...
    for root, dirs, files in os.walk(dir_path):
        for file in files:
            if file.endswith('.mp4'):
                fileName = str(file)
                fileLabel = myfont.render(fileName, 1, (0,0,0))
                screen.blit(txt_filesFound, centerFileText(txt_filesFound))
                screen.blit(fileLabel,centerText(fileLabel))
                pygame.display.flip()
